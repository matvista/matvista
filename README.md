...
The app opens on a landing page introducing the thirteen modules, indexed by the same four
...
modules chapter: thirteen panels, one per module, each plotted from that module's own model —
...
no longer equal: Properties runs four deep where the others run three, and the grid draws a
seam only as far as the deeper of the two columns it separates and a row rule only across
the columns that have a row there.
...
paint is the index chunk and the stylesheet and nothing else: **93.02 kB gzipped** — index
84.20 plus a render-blocking 8.82. The thirteenth module cost 0.77 kB of that, which is its
landing card, its signature mark and its nav entry; the module itself is a 5.86 kB chunk
that only a reader who opens it ever fetches.
